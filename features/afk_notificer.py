...
async def set_afk_status(user_id: str | int, status: str, previous_nick: str, server_id: str | int):
...
async def get_afk_status(user_id: str) -> str | None:
    afk_data = await collection.find_one({"_id": str(user_id)})
    if afk_data and "status" in afk_data:
        return afk_data["status"]
    return None


async def get_previous_nick(user_id: str) -> str | None:
    afk_data = await collection.find_one({"_id": str(user_id)})
...
async def get_server_id(user_id: str) -> str | None:
    afk_data = await collection.find_one({"_id": str(user_id)})
...
async def clear_afk_status(user_id: str):
    await collection.delete_one({"_id": str(user_id)})
...
async def on_message(message: discord.Message):
    ...
    author_afk_status = await get_afk_status(str(message.author.id))
    guild_id = await get_server_id(str(message.author.id))
    if not message.guild or not isinstance(message.author, discord.Member):
        return
    if author_afk_status and guild_id and str(message.guild.id) == guild_id:
        await clear_afk_status(str(message.author.id))

        try:
            n = await get_previous_nick(str(message.author.id))
            s = await get_afk_status(str(message.author.id))
    ...
    for mention in message.mentions:
        afk_status = await get_afk_status(str(mention.id))
        if afk_status:
            response = get_string_by_id(loca_sheet, "afk_response").format(
                await get_previous_nick(str(mention.id)), afk_status
            )
            await message.channel.send(response)