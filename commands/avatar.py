...
async def slash_command_listener(ctx: discord.Interaction, user: discord.User, server_avatar: bool = True):
    ...
    if avatar is not None:
        embed = discord.Embed(
            title=get_string_by_id(loca_sheet, "command_avatar_embed_title"),
            description=get_string_by_id(loca_sheet, "command_avatar_embed_desc").format(user),
            color=0x03e3fc,
            type="image"
        )
        ...
        await ctx.followup.send(get_string_by_id(loca_sheet, "command_avatar_noavatar"))
